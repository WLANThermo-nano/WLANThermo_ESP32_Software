  ...
  <div class="pure-g m-4">
    ...
    <template v-if="!isEditingProfile">
      <div class="config-form-container pure-u-1-1 pure-u-md-1-1 pure-u-lg-1-1 mt-2"
        v-for="(pm, index) in $v.pitmaster.pm.$each.$iter" :key="index">
          ...
          <form>
            <div class="form-group">
              <select v-model="pm.typ.$model">
                <option v-for="pitmasterType in pitmaster.type" :key="pitmasterType" :value="pitmasterType">
                  {{ pitmasterType }}
                </option>
              </select>
            ...
            <div class="form-group" v-if="pm.typ.$model === 'auto'">
              <select v-model="pm.channel.$model">
                <option v-for="(channel, index) in channels" :key="index" :value="index + 1">
          ...
          <form>
            <div class="form-group">
              <select v-model="editingProfile.aktor">
                <option v-for="(act, index) in actuators" :key="index" :value="index">
                  {{ act }}
                </option>
              </select>
              ...
              <div class="pure-u-1-2 control">
                <div class="form-group" :class="{ 'error': $v.editingProfile.DCmmin.$invalid }">
                  ...
                  <div class="error-prompt" v-if="$v.editingProfile.DCmmin.$invalid">
                    {{ $t('v_must_between', {
                      min: $v.editingProfile.DCmmin.$params.between.min, max:
                        $v.editingProfile.DCmmin.$params.between.max
                    }) }}
                  </div>
                </div>
              </div>
              <div class="pure-u-1-2 control">
                <div class="form-group" :class="{ 'error': $v.editingProfile.DCmmax.$invalid }">
                  ...
                  <div class="error-prompt" v-if="$v.editingProfile.DCmmax.$invalid">
                    {{ $t('v_must_between', {
                      min: $v.editingProfile.DCmmax.$params.between.min, max:
                        $v.editingProfile.DCmmax.$params.between.max
                    }) }}
                  </div>
                </div>
              </div>
              ...
              <div class="pure-u-1-2 control">
                <div class="form-group" :class="{ 'error': $v.editingProfile.SPmin.$invalid }">
                  ...
                  <div class="error-prompt" v-if="$v.editingProfile.SPmin.$invalid">
                    {{ $t('v_must_between', {
                      min: $v.editingProfile.SPmin.$params.between.min, max:
                        $v.editingProfile.SPmin.$params.between.max
                    }) }}
                  </div>
                </div>
              </div>
              <div class="pure-u-1-2 control">
                <div class="form-group" :class="{ 'error': $v.editingProfile.SPmax.$invalid }">
                  ...
                  <div class="error-prompt" v-if="$v.editingProfile.SPmax.$invalid">
                    {{ $t('v_must_between', {
                      min: $v.editingProfile.SPmax.$params.between.min, max:
                        $v.editingProfile.SPmax.$params.between.max
                    }) }}
                  </div>
                </div>
              </div>
            ...
            <form>
              ...
              <div class="pure-u-1-3 control">
                <div class="form-group" :class="{ 'error': $v.editingProfile.Kp.$invalid }">
                  ...
                  <div class="error-prompt" v-if="$v.editingProfile.Kp.$invalid">
                    {{ $t('v_must_between', {
                      min: $v.editingProfile.Kp.$params.between.min, max:
                        $v.editingProfile.Kp.$params.between.max
                    }) }}
                  </div>
                </div>
              </div>
              <div class="pure-u-1-3 control">
                <div class="form-group" :class="{ 'error': $v.editingProfile.Ki.$invalid }">
                  ...
                  <div class="error-prompt" v-if="$v.editingProfile.Ki.$invalid">
                    {{ $t('v_must_between', {
                      min: $v.editingProfile.Ki.$params.between.min, max:
                        $v.editingProfile.Ki.$params.between.max
                    }) }}
                  </div>
                </div>
              </div>
              <div class="pure-u-1-3 control">
                <div class="form-group" :class="{ 'error': $v.editingProfile.Kd.$invalid }">
                  ...
                  <div class="error-prompt" v-if="$v.editingProfile.Kd.$invalid">
                    {{ $t('v_must_between', {
                      min: $v.editingProfile.Kd.$params.between.min, max:
                        $v.editingProfile.Kd.$params.between.max
                    }) }}
                  </div>
                </div>
              </div>
              <div class="form-group control-at-top" :class="{ 'error': $v.editingProfile.jp.$invalid }">
                ...
                <div class="error-prompt" v-if="$v.editingProfile.jp.$invalid">
                  {{ $t('v_must_between', {
                    min: $v.editingProfile.jp.$params.between.min, max:
                      $v.editingProfile.jp.$params.between.max
                  }) }}
                </div>
              </div>
            </form>
